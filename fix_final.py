...
# 2. Add voice CSS before /* Print */
voice_css = '''
/* 语音按钮 */
.voice-btn {
  position: absolute; right: 12px; top: 50%; transform: translateY(-50%);
  width: 36px; height: 36px; border-radius: 50%; border: none;
  background: var(--sage-light); color: var(--sage-dark); cursor: pointer;
  font-size: 1.1rem; display: flex; align-items: center; justify-content: center;
  transition: var(--transition); z-index: 2;
}
.voice-btn:hover { background: var(--sage); color: white; }
.voice-btn.listening { background: var(--cinnabar); color: white; animation: pulse-voice 1s ease-in-out infinite; }
@keyframes pulse-voice { 0%,100% { box-shadow: 0 0 0 0 rgba(199,91,58,0.3); } 50% { box-shadow: 0 0 0 12px rgba(199,91,58,0); } }
.voice-status { text-align: center; font-size: 0.82rem; color: var(--cinnabar); margin-top: 8px; min-height: 20px; }
.search-wrap { position: relative; }
.search-input { padding-right: 48px; }
/* 语音朗读按钮 */
.speak-btn {
  display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px;
  border-radius: 50px; border: 1px solid var(--sage); background: var(--sage-light);
  color: var(--sage-dark); cursor: pointer; font-size: 0.8rem;
  font-family: var(--font-body); transition: var(--transition); margin-left: 8px; vertical-align: middle;
}
.speak-btn:hover { background: var(--sage); color: white; }
.speak-btn.speaking { background: var(--cinnabar); color: white; border-color: var(--cinnabar); animation: pulse-voice 1s ease-in-out infinite; }'''
html = html.replace('/* ── Print ── */', voice_css + '\n\n/* ── Print ── */')

# 3. Add voice button in search
old_search = '<input type="text" class="search-input" id="searchInput" placeholder="搜索疾病名称或症状…" autocomplete="off">'
new_search = '<input type="text" class="search-input" id="searchInput" placeholder="搜索疾病名称或症状…" autocomplete="off">\n    <button class="voice-btn" id="voiceSearchBtn" title="语音搜索">🎤</button>'
html = html.replace(old_search, new_search)

# 4. Add voice status div
html = html.replace('</div>\n</div>\n\n<nav', '</div>\n  <div class="voice-status" id="voiceStatus"></div>\n</div>\n\n<nav')

# 5. Replace category buttons with all 10 categories
old_cats = '''  <button class="category-btn active" data-category="all"><span class="icon">📋</span>全部</button>
  <button class="category-btn" data-category="heart"><span class="icon">❤️</span>心脑血管</button>
  <button class="category-btn" data-category="brain"><span class="icon">🧠</span>脑血管</button>
  <button class="category-btn" data-category="stomach"><span class="icon">🍽️</span>胃肠病</button>'''

new_cats = '''  <button class="category-btn active" data-category="all"><span class="icon">📋</span>全部</button>
  <button class="category-btn" data-category="heart"><span class="icon">❤️</span>心脑血管</button>
  <button class="category-btn" data-category="brain"><span class="icon">🧠</span>脑血管</button>
...
# 6. Add voice JS before the final </script>
voice_js = '''
...
function speakDisease(diseaseId) {
  var d = diseases.find(function(d) { return d.id === diseaseId; });
  if (!d) return;
  ...
  var zhVoice = voices.find(function(v) { return v.lang.startsWith("zh-CN") || v.lang.startsWith("zh-TW") || v.lang.startsWith("zh"); });
  if (zhVoice) utterance.voice = zhVoice;
  var btn = document.getElementById("speak-btn-" + diseaseId);
  if (btn) { btn.classList.add("speaking"); btn.textContent = "🔊 朗读中…"; }
  utterance.onend = function() { if (btn) { btn.classList.remove("speaking"); btn.textContent = "🔊 听语音"; } };
  utterance.onerror = function() { if (btn) { btn.classList.remove("speaking"); btn.textContent = "🔊 听语音"; } };
  window.speechSynthesis.speak(utterance);
}

// 重写 renderDiseases，添加朗读按钮
var _origRender = renderDiseases;
renderDiseases = function() {
  _origRender();
  document.querySelectorAll(".disease-name").forEach(function(el) {
    ...
    if (!document.getElementById("speak-btn-" + diseaseId)) {
      ...
      btn.textContent = "🔊 听语音";
      btn.title = "语音朗读";
      btn.onclick = function(e) { e.stopPropagation(); speakDisease(diseaseId); };
      el.appendChild(btn);
    }
  });
};
...
# Find the last renderDiseases(); and insert before it
# Actually find the last </script>
last_script = html.rfind('</script>')
html = html[:last_script] + voice_js + '\n</script>' + html[last_script+9:]
...
# Verify
for chk in ['voice-btn', 'speak-btn', 'voiceSearchBtn', 'speakDisease', 'og:title', 'metabolic', 'respiratory', 'nerve', 'bone', 'mental', 'sensory', 'urinary', 'tumor']:
    cnt = html.count(chk)
    print(f'{chk}: {cnt}')
